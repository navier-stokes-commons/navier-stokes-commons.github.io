...
def canonical_fields(x,y,t,A,nu):
    f=sim['fields']; v={'x':x,'y':y,'t':t}; p={'A':A,'nu':nu}
    return expr(f['u_x'],v,p),expr(f['u_y'],v,p),expr(f['p'],v,p)
...
def oracle_fields(x,y,t,A,nu):
    d=math.exp(-2*nu*t)
    return A*d*math.sin(x)*math.cos(y),-A*d*math.cos(x)*math.sin(y),(A*A/4)*math.exp(-4*nu*t)*(math.cos(2*x)+math.cos(2*y))
...
A=1.17;nu=.043;worst_div=worst_res=0.0
...
# The exact field is intentionally isolated from the 2026 schematic flagship.
home=(P/'en/index.html').read_text() if (P/'en/index.html').exists() else ''
ref=(P/'en/reference-flow/index.html').read_text() if (P/'en/reference-flow/index.html').exists() else ''
if 'data-flow-chamber' in home: errs.append('exact Taylor flow leaked back into 2026 flagship home')
for token,label in [('data-flow-chamber','simulation root'),('data-flow-model','canonical expression model'),('data-flow-canvas','progressive canvas'),('data-flow-static','static fallback'),('data-flow-state','parallel textual state'),('data-flow-controls','controls'),('not a reconstruction of the 2026 three-dimensional singular construction','scope boundary')]:
    if token not in ref: errs.append('reference-flow page missing '+label)
if '<canvas' in ref and 'aria-hidden="true"' not in ref: errs.append('reference canvas must not substitute for accessible state')
...
if errs:
    ...
    raise SystemExit(1)
print(f'FLOW_CHAMBER_AUDIT_PASS worst_div={worst_div:.2e} worst_momentum_residual={worst_res:.2e} energy_quadrature=exact-within-tolerance placement=isolated-reference-flow')
